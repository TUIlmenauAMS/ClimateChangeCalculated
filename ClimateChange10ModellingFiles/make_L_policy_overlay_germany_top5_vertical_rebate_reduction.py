...
def make_plot(
    ...
    pol["Policy_Title"].str.contains(
        "rebate reduction|bonus cut|subsidy cut", case=False, na=False
          )
       ]

    supportive = pol_sorted[
       ~pol_sorted.index.isin(harmful.index)
    ].head(top_n - len(harmful))

    top = (
       pd.concat([supportive, harmful])
         .drop_duplicates(subset=["Policy_Title", "Year"])
         .sort_values("Year")
    )
